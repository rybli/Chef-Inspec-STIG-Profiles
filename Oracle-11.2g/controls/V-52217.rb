...
control "V-52217" do
	...
	desc "When it comes to DoS attacks, most of the attention is paid to ensuring that systems and applications are not victims of these attacks.
...
This calls for inspection of application source code, which will require collaboration with the application developers. It is recognized that in many cases, the database administrator (DBA) is organizationally separate from the application developers and may have limited, if any, access to source code. Nevertheless, protections of this type are so important to the secure operation of databases that they must not be ignored. At a minimum, the DBA must attempt to obtain assurances from the development organization that this issue has been addressed and must document what has been discovered.true"
	impact 0.5
	tag "check": "Review DBMS settings and custom database code to determine whether the DBMS or database application code could be used to launch DoS attacks. If the DBMS or custom database code would facilitate DoS-style attacks against other information systems, this is a finding.


The Listener is the key for a denial of service attack.  The threat is that the database is flooded with connection requests, preventing additional valid connections which will ultimately result in the failure of the listener.

Check to insure the appropriate steps to secure the Oracle Listener are in place at your site.  (Please refer to the Fix for more detail on implementing these protections.)"
	tag "fix": "Configure DBMS settings to restrict functionality that could be used to initiate DoS attacks.
