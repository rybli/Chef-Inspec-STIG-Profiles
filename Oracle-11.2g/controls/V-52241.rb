...
control "V-52241" do
	...
	desc "Application recovery and reconstitution constitutes executing an information system contingency plan comprised of activities that restore essential missions and business functions. 
...
false"
	impact 0.5
	tag "check": "Review DBMS recovery procedures and technical system features to determine if mechanisms exist and are in place to specify use of trusted files during DBMS recovery. If recovery procedures do not exist or are not sufficient to ensure recovery is done in a secure and verifiable manner, this is a finding.
...
Review the database backup strategy with the system administrator.  Consider using Oracle RMAN with an encrypted backup to insure you have files that can be trusted not to be compromised."
	tag "fix": "Implement DBMS recovery procedures and employ technical system features to specify trusted files during DBMS recovery.  Test the solution and review the site-specific criteria to ensure that the backup and recovery process uses trusted files.
