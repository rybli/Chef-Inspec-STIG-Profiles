...
Authentication modules with weak encryption could allow an attacker to gain access to data stored in the database and to the administration settings of the DBMS.false"
	impact 0.5
	tag "check": "Check the following settings to see if FIPS 140-2 authentication/encryption is configured. If encryption is required but not configured, check with the DBA and SYSTEM Administrator to see if other mechanisms or third-party cryptography products are deployed for authentication.
...
If the line "SSLFIPS_140=TRUE" is not found in fips.ora, or the file does not exist, this is a finding."
	tag "fix": "Utilize NIST-validated FIPS 140-2-compliant cryptography for all authentication mechanisms.
