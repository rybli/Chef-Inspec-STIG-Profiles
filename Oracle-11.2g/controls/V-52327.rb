...
Unsupported software versions are not patched by vendors to address newly discovered security versions. An unpatched version is vulnerable to attack.false"
	impact 0.5
	tag "check": "Follow the vendor instruction for determining the product version number. View the vendor-provided list of supported versions. To be considered supported, the vendor must report that the version is supported by security patches to reported vulnerabilities. If the security patch support for the installed version cannot be determined or the version is not shown as supported, this is a finding.
...
If the latest available patch is not installed, this is a finding."
	tag "fix": "Upgrade the DBMS to a vendor-supported version.
