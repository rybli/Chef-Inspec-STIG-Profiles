...
Using protocols deemed non-secure would compromise the ability of the DBMS to operate in a secure fashion. The database must be able to disable network protocols deemed non-secure.false"
	impact 0.5
	tag "check": "Review the PPSM Technical Assurance List to acquire an up-to-date list of network protocols deemed non-secure.
...
Protocol: TCP/IP with SSL  Parameter: PORT      Notes: Specify the listening port number.
	...
	tag "fix": "Disable any network protocol listed as non-secure in the PPSM documentation.
