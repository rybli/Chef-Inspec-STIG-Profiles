...
Note:  this does not require that all databases be encrypted.  It specifies that if encryption is required, then the implementation of the encryption must satisfy the prevailing standards.false"
	impact 0.5
	tag "check": "If encryption is not required for the database, this is not a finding.
...
Determine whether the Oracle DBMS software is at version 11.2.0.4 with the January 2014 CPU (or above).  If it is not, this is a finding."
	tag "fix": "Implement required cryptographic protections using cryptographic modules complying with applicable federal laws, Executive Orders, directives, policies, regulations, standards, and guidance.
