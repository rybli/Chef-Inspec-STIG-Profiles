...
control 'V-77249' do
  ...
  if input('sensitive_system') == 'true' || nil
   ...
   payload = json( command: 'Get-ProcessMitigation -Name PPTVIEW.EXE | Select Payload | ConvertTo-Json').params
       describe 'Override Payload ExportAddressFilter, Override Payload ExportAddressFilterPlus, Override ImportAddressFilter, Override EnableRopStackPivot, Override EnableRopCallerCheck, and Override EnableRopSimExec are required to be false on Microsoft Office PowerPoint Viewer' do
        subject { payload }
