...
control 'V-77221' do
  ...
  desc 'check', "This is NA prior to v1709 of Windows 10.
      ...
      TThe PowerShell command produces a list of mitigations; only those with a 
  ...
  if input('sensitive_system') == 'true' || nil
    ...
    payload = json( command: 'Get-ProcessMitigation -Name INFOPATH.EXE | Select Payload | ConvertTo-Json').params
       describe 'Override Payload ExportAddressFilter, Override Payload ExportAddressFilterPlus, Override ImportAddressFilter, Override EnableRopStackPivot, Override EnableRopCallerCheck, and Override EnableRopSimExec are required to be false on Microsoft InfoPath' do
        subject { payload }
        ...
        its(['OverrideEnableRopStackPivot']) { shoul eq 'false' }
