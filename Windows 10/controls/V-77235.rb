...
control 'V-77235' do
  ...
  if input('sensitive_system') == 'true' || nil
    ...
    aslr = json( command: 'Get-ProcessMitigation -Name OneDrive.exe | Select Aslr | ConvertTo-Json').params
       describe 'Alsr OverRide Force Relocate Images are required to be NOTSET on Microsoft Onedrive' do
        subject { aslr }
    ...
    imageload = json( command: 'Get-ProcessMitigation -Name OneDrive.exe | Select ImageLoad | ConvertTo-Json').params
       describe 'Override ImageLoad Block Remote Image Loads is required to be false on Microsoft Onedrive' do
        subject { imageload }
        its(['OverrideBlockRemoteImages']) { should eq 'false' }
       end
    payload = json( command: 'Get-ProcessMitigation -Name OneDrive.exe | Select Payload | ConvertTo-Json').params
       describe 'Override Payload ExportAddressFilter, Override Payload ExportAddressFilterPlus, Override ImportAddressFilter, Override EnableRopStackPivot, Override EnableRopCallerCheck, and Override EnableRopSimExec are required to be false on Microsoft Onedrive' do
        subject { payload }
