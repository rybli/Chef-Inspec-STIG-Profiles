...
control 'V-77263' do
  ...
  desc 'check', "This is NA prior to v1709 of Windows 10.
      ...
      required status of are listed here. If the PowerShell command does not
  ...
  if input('sensitive_system') == 'true' || nil
   ...
   payload = json( command: 'Get-ProcessMitigation -Name WINWORD.EXE | Select Payload | ConvertTo-Json').params
       describe 'Override Payload ExportAddressFilter, Override Payload ExportAddressFilterPlus, Override ImportAddressFilter, Override EnableRopStackPivot, Override EnableRopCallerCheck, and Override EnableRopSimExec are required to be false on Microsoft Office Word' do
        subject { payload }
