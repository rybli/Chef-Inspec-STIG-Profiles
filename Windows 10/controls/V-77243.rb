  ...
  if input('sensitive_system') == 'true' || nil
   ...
   dep = json( command: 'Get-ProcessMitigation -Name OUTLOOK.EXE | Select DEP | ConvertTo-Json').params
    ...
    payload = json( command: 'Get-ProcessMitigation -Name OUTLOOK.EXE | Select Payload | ConvertTo-Json').params
       describe 'Override Payload ExportAddressFilter, Override Payload ExportAddressFilterPlus, Override ImportAddressFilter, Override EnableRopStackPivot, Override EnableRopCallerCheck, and Override EnableRopSimExec are required to be false on Microsoft Office Outlook' do
        subject { payload }
