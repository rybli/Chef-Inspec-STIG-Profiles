...
control 'V-77233' do
  ...
  if input('sensitive_system') == 'true'
    ...
    payload = json( command: 'Get-ProcessMitigation -Name MSPUB.EXE | Select Payload | ConvertTo-Json').params
       describe 'Override Payload ExportAddressFilter, Override Payload ExportAddressFilterPlus, Override ImportAddressFilter, Override EnableRopStackPivot, Override EnableRopCallerCheck, and Override EnableRopSimExec are required to be false on Microsoft Office Publisher' do
        subject { payload }
