...
Audit records can be generated from various components within the information system (e.g., module or policy filter)."
  impact 0.5
  tag "check": "Verify the operating system generates audit records when successful/unsuccessful attempts to use the 'chown' syscall occur.
...
"
  ...
  describe auditd do
    its('lines') { should include %r(-a always,exit -F arch=b32 -S chown -F auid>=1000 -F auid!=4294967295 -k perm_mod) }
    its('lines') { should include %r(-a always,exit -F arch=b64 -S chown -F auid>=1000 -F auid!=4294967295 -k perm_mod) }
  end
