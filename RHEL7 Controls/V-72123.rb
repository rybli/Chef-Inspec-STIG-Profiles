...
Audit records can be generated from various components within the information system (e.g., module or policy filter)."
  impact 0.5
  tag "check": "Verify the operating system generates audit records when successful/unsuccessful attempts to use the 'creat' syscall occur.
...
If the output does not produce rules containing '-F exit=-EACCES', this is a finding."
  tag "fix": "Configure the operating system to generate audit records when successful/unsuccessful attempts to use the 'creat' syscall occur.
...
The audit daemon must be restarted for the changes to take effect."
  ...
  describe auditd do
    its('lines') { should include %r(-a always,exit -F arch=b32 -S creat F exit=-EPERM -F auid>=1000 -F auid!=4294967295 -k access) }
    its('lines') { should include %r(-a always,exit -F arch=b32 -S creat -F exit=-EACCES -F auid>=1000 -F auid!=4294967295 -k access) }
    its('lines') { should include %r(-a always,exit -F arch=b64 -S creat -F exit=-EPERM -F auid>=1000 -F auid!=4294967295 -k access) }
    its('lines') { should include %r(-a always,exit -F arch=b64 -S creat -F exit=-EACCES -F auid>=1000 -F auid!=4294967295 -k access) }
  end
