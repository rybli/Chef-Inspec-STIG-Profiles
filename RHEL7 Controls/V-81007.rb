...
## TODO: Might be able to use https://www.inspec.io/docs/reference/resources/grub_conf/
   describe file('/boot/efi/EFI/redhat/grub.cfg') do
     its('content') { shoudl match %r{^set.superusers.*=.*\"root\"$}) }
     its('content') { shoudl match %r{^export.superusers$}) }
   end
   describe file('/boot/efi/EFI/redhat/user.cfg') do
     its('content') { shoudl match %r{^GRUB2_PASSWORD=grub.pbkdf2.sha512.*$) }
   end
end
