...
specific hosts.   "
  ...
  # Check to see if Firewall is enabled
  if package('firewalld').installed? && service('firewalld.service').running? && service('firewalld.service').enabled?
    describe command('firewall-cmd --get-default-zone') do
  end
  else
    # Check is tcpwrappers is installed and being used.
  end
end
