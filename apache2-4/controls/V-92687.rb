...
Generating session IDs that are at least 128 bits (16 bytes) in length will cause an attacker to take a large amount of time and resources to guess, reducing the likelihood of an attacker guessing a session ID.false"
	impact 0.5
	tag "check": "Review the web server documentation and deployed configuration to determine the length of the generated session identifiers.
...
If the 'SessionCryptoCipher' is not used or 'SessionCryptoCipher' is not set to 'aes256', this is a finding."
	tag "fix": "Configure the web server to generate session identifiers that are at least 128 bits in length.
...
Restart Apache: apachectl restart"
	...
	describe command('httpd -M | grep -i "session_crypto"') do
		its('stdout') { should eq ' session_crypto_module (shared)' }
	end
	describe apache_conf() do
		its('SessionCryptoCipher') { should eq 'aes256' }
	end
