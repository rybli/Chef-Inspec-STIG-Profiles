...
Service accounts or system accounts that have no logon capability do not need to have passwords set or changed.false"
	impact 0.5
	tag "check": "Identify the account that is running the 'httpd' process:
...
If the account has a valid login shell and a password defined, this is a finding."
	tag "fix": "Update the /etc/passwd file to assign the account used to run the 'httpd' process an invalid login shell such as '/sbin/nologin'.
...
passwd: Success"
	...
	describe command('ps -ef | grep -i httpd | grep -v grep | cut -d" " -f1') do
		its('stdout') { should eq 'apache' }
	end
	describe.one do
		describe command('cut -d: -f1,7') do
			its('stdout') { should eq 'apache:/sbin/nologin' }
		end
		describe command('cut -d: -f1,2 /etc/shadow | grep -i apache') do
			its('stdout') { should eq 'apache:!!' }
		end
	end
